...
# will set to 1 if SUMO simulation failed
SUMO_DONE=0

# will set to 1 if conversion xml->csv failed
CSV_DONE=0
...
then
    ...
    logit "Skipping conversion to CSV"
    CSV_DONE=1
fi
...
load_db() {
    ...
    # https://unix.stackexchange.com/a/392973 modified to have more argument
    # NOTE. -exec + will try to put as many filenames as possible in a single run
    # then our code iterates over {@:3}-- whatevere was put therein
...
then
    ...
    if [ $CSV_DONE -eq 0 ] && [ $S3_DONE -eq 0 ] && [ $DB_DONE -eq 0 ]
    then
    ...
    then rm $TMP_DIR/${SUMO_MODEL_PREFIX}.${SUMO_OUTPUT_FILE}.csv.0*
    fi
fi
